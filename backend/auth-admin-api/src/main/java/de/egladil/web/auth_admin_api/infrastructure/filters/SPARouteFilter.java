...
// (c) Heike Winkelvoß
// =====================================================
package de.egladil.web.auth_admin_api.infrastructure.filters;
...
public class SPARouteFilter {
	...
	private static final String API_PREFIX = "/auth-admin-api/";

	private static final String APP_PLUS_API_PREFIX = "/bv-admin-app" + API_PREFIX;

	private static final String DEFAULT_APP = "/bv-admin-app/";
		...
		if (path.startsWith(APP_PLUS_API_PREFIX)) {
			...
			if (this.doesNotNeedRedirect(path)) {
				...
				if (path.startsWith(DEFAULT_APP)) {
					...
					// I0094: deep-Angular-Router-Links (z.B. /bv-admin-app/users/) müssen zur SPA Grund-URL
					// (/bv-admin-app/) umgeleitet werden. Danach übernimmt wieder das Angular-Routing
