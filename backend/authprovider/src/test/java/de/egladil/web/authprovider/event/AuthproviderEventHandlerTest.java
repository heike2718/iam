...
public class AuthproviderEventHandlerTest {
    ...
    void should_handleEventHandleProperly_when_userDeleted() {
        ...
        DefaultEmailDaten emailDaten = createEmailDaten(MinikaengurukontenMailKontext.CONFIRMATION_EXPIRED,
                resourceOwner);
    ...
    private DefaultEmailDaten createEmailDaten(final MinikaengurukontenMailKontext kontext,
            final ResourceOwner resourceOwner) {

        return new MinikaengurukontenInfoStrategie(ResourceOwnerEventPayload.createFromResourceOwner(resourceOwner),
                kontext, "DEV", monitoringMailEmpfaenger).createEmailDaten(kontext.name());
    }
}
