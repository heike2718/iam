...
/**
 * BenutzerprofilResource ist der Endpoint, der vom profileservice verwendet
 * wird, um die Daten eines Users zu ändern.
 */
@RequestScoped
@Path("api/profiles")
...
public class BenutzerprofilResource {
    ...
    public Response changePassword(@Valid final ChangeProfilePasswordPayload payload) {
        ...
        try {
            ...
            ResponsePayload responsePayload = changePasswordService
                    .changePassword(payload.getUuid(), payload.getPasswordPayload());
    ...
    /**
     * Falls Mailadresse vergeben, dann 910, Falls Loginname vergeben, dann 911,
     * falls beides vergeben, dann 912
     *
     * @param payload
     * @return
     */
    ...
    /**
     * Gibt den User zurück. Die UUID ist im Container ist nicht mehr klar, was ich
     * mit der USER_ID- Property im ContainerRequestContext vorhatte.
     *
     * @param crc
     * @param userId
     * @return
    ...
    @APIResponse(
            ...
            content = @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ResponsePayload.class)))
    public Response deleteUser(@Valid final SelectProfilePayload selectProfilePayload) {
    ...
    void verifyClientId(@Valid final OAuthClientCredentials clientCredentials) {
        ...
        if (!permittedClientId.equals(clientCredentials.getClientId())) {
            ...
            LOG
                    .warn("Nicht erlaubter Zugriff auf Profil-Resource mit ClientId "
