...
/**
 * ClientResource stellt REST-Endpoints für die Clients zur Verfügung, mit denen
 * sie sich die nicht geheimen Infos abholen können, die über sie in der DB
 * stehen. Zur Authentisierung dient die clientId und eine redirect-URL.
 */
@RequestScoped
@Path("api/clients")
...
public class ClientResource {
    ...
    @Parameters({
            @Parameter(
                    ...
                    name = "accessToken",
                    description = "Ein zuvor über POST authprovider/api/clients/client/accesstoken angefordertes OTT.",
                    required = true),
            @Parameter(
                    in = ParameterIn.QUERY,
                    name = "redirectUrl",
                    description = "Die Redirect-URL für den anfragenden Client. Ist das Secret in der Kommunikation.",
                    required = true),
            @Parameter(
                    in = ParameterIn.QUERY,
                    name = "state",
                    description = "hat seine Bedeutung irgendwie verloren, hängt aber in allen Client-Anwendungen drin."), })
    @APIResponse(
            name = "OKResponse",
            responseCode = "200",
            content = @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ClientInformation.class)))
    @APIResponse(
    ...
    /**
     * Authentisiert den Client und erzeugt ein OAuthAccessTokenPayload, welches ein
     * Client-AccessToken enthält.
     *
    ...
    @APIResponse(
            ...
            content = @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = OAuthAccessTokenPayload.class)))
    @APIResponse(
            name = "BadRequestResponse",
            responseCode = "400",
            description = "fehlgeschlagene Input-Validierung",
    ...
    public Response authenticateClient(@Valid final OAuthClientCredentials clientCredentials) {
        ...
        try {
            ...
            throw new AuthException("Der Client mit ID=" + StringUtils.abbreviate(clientCredentials.getClientId(), 15)
