 ...
 * <br>
 * <code>Beispiel:</code> Ein Lehrer in der Minikänguru-Anwendung, der Admin der
 * Minikänguru-Admin-Anwendung, jemand, der die Checklistenapp verwenden
 * darf,...
 */
@Entity
@Table(name = "USERS")
@NamedQueries({
        @NamedQuery(
                name = ResourceOwner.FIND_BY_EMAIL,
                query = "select o from ResourceOwner o where lower(o.email) = :email"),
        @NamedQuery(
                name = ResourceOwner.FIND_BY_EMAIL_LIKE,
                query = "select o from ResourceOwner o where lower(o.email) like :email"),
        @NamedQuery(
                name = ResourceOwner.FIND_BY_LOGINNAME,
                query = "select o from ResourceOwner o where o.loginName = :loginName"),
        @NamedQuery(
                name = ResourceOwner.FIND_BY_LOGINNAME_LIKE,
                query = "select o from ResourceOwner o where lower(o.loginName) like :loginName"),
        @NamedQuery(name = ResourceOwner.FIND_BY_UUID, query = "select o from ResourceOwner o where o.uuid = :uuid"),
        @NamedQuery(
                name = ResourceOwner.FIND_BY_UUID_LIKE,
                query = "select o from ResourceOwner o where o.uuid LIKE :uuid"),
        @NamedQuery(
                name = ResourceOwner.FIND_OTHER_BY_EMAIL,
                query = "select o from ResourceOwner o where o.uuid != :uuid and lower(o.email) = :email"),
        @NamedQuery(
                name = ResourceOwner.FIND_OTHER_BY_LOGINNAME,
                query = "select o from ResourceOwner o where o.uuid != :uuid and lower(o.loginName) = :loginName"),
        @NamedQuery(
                name = ResourceOwner.FIND_BY_ACTIVATION_AND_BANNED_STATE,
                query = "select o from ResourceOwner o where o.aktiviert != :aktiviert and o.bannedForMails = :bannedForMails"), })
public class ResourceOwner implements AuthProviderEntity {
    ...
    public String toString() {
        ...
        return "ResourceOwner [uuid=" + StringUtils.abbreviate(uuid, 11) + ", fullName=" + getFullName()
                + ", loginName=" + loginName + ", email=" + email + "]";
    }

    public String toLogString() {

        return "ResourceOwner [uuid=" + StringUtils.abbreviate(uuid, 11) + ", fullName=" + getFullName()
                + ", loginName=" + loginName + ", email=" + StringUtils.abbreviate(email, 9) + "]";
    }
