...
public class AuthCryptoServiceImpl implements AuthCryptoService {
    ...
    /**
     * übernomemen aus org.apache.shiro.crypto.support.hashes.argon2.Argon2Hash.
     * Muss bei updates von shiro immer mit getestet werden!!!
     */
    ...
    /**
     * Verifiziert die LoginSecrets. Falls sie noch mit SHA-256 berechnet sind,
     * werden sie umgehashed.
     *
     * @param loginSecrets
     * @param password
     * @return
    ...
    boolean verifyLoginSecrets(final LoginSecrets loginSecrets, final char[] password) {
        ...
        try {
            ...
            if (CryptoAlgorithm.SHA_256 == loginSecrets.getCryptoAlgorithm()) {
                ...
                matches = new LegacyPasswordService(passwordConfig)
                        .verifyPassword(password, loginSecrets.getPasswordhash(), loginSecrets.getSalt().getWert());
