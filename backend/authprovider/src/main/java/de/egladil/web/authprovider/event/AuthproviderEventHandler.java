...
public class AuthproviderEventHandler {
    ...
    public void handleEvent(@Observes final AuthproviderEvent event) {
        ...
        if (event != null) {
            ...
            if (event.writeToEventStore()) {
                ...
                StoredEvent storedEvent = StoredEvent
            ...
            if (mkGatewayResponse.getStatus() != 200) {
                ...
                LOGGER
                        .error("Status {} vom mk-gateway beim Senden des DeleteUserCommands {} ",
                                mkGatewayResponse.getStatus(), resourceOwner.getUuid());
                this.sendeInfoAnMichQuietly(MinikaengurukontenMailKontext.SYNC_FAILED, resourceOwner);
    ...
    private void propagateUserCreated(final Object payload, final String syncToken) {
        ...
        String fullName = StringUtils
                .isAllBlank(new String[] { resourceOwner.getVorname(), resourceOwner.getNachname() }) ? null
        ...
        try (Response mkGatewayResponse = mkGateway.propagateUserCreated(command)) {
            ...
            if (mkGatewayResponse.getStatus() != 200) {
                ...
                LOGGER
                        .error("Status {} vom mk-gateway beim Senden des CreateUserCommand {} ",
                                mkGatewayResponse.getStatus(), command);
                this.sendeInfoAnMichQuietly(MinikaengurukontenMailKontext.SYNC_FAILED, resourceOwner);
                throw new PropagationFailedException(applicationMessages.getString("createUser.propagation.failure"));
            }
            ...
            LOGGER
                    .error("ProcessingException beim Propagieren des CreateUserEvents ans mk-gateway: {}",
                            e.getMessage(), e);
            ...
            LOGGER
                    .error("WebApplicationException beim Propagieren des CreateUserEvents: status={} - {}",
