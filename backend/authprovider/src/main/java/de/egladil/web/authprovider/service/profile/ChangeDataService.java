...
public class ChangeDataService {
    ...
    /**
     * Ändert die Daten zu gegebenem ResourceOwner, falls es dadurch nicht zu
     * Eindeutigkeitskonflikten kommt.
     *
    ...
    public User changeData(final String uuid, final ProfileDataPayload payload) {
        ...
        DuplicateAttributeType checkOutcome = resourceOwnerService
                .changeLoginNameAndEmailAllowed(payload.getLoginName(), payload.getEmail(), uuid);
    ...
    /**
     * Einfache Heuristik: wenn resourceOwner eine banned Mailadresse hat sich die
     * neue Mailadresse nach dem letzten @ von dem Teil der alten Mailadresse nach
     * dem letzten @ unterscheidet, hat sich die Domain geändert und man kann das
     * Entbannen versuchen.
     *
    ...
    boolean getResetBanFlag(ResourceOwner resourceOwner, String email) {
        ...
        LOG
                .info("USER {}: oldDomain={}, newDomain={} => reset the ban flag",
                        StringUtils.abbreviate(resourceOwner.getUuid(), 11), oldDomain, newDomain);
