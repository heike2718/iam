...
public class BenutzerResource {
    ...
    @RolesAllowed({ "AUTH_ADMIN" })
    @Operation(
            operationId = "flags eines Benutzers ändern",
            summary = "Setzt für den gegebenen Benutzer die neuen Flags")
    ...
    @APIResponse(
            name = "Forbiddeb",
            responseCode = "403",
            description = "einige Benutzerkonten dürfen nicht gelöscht werden.",
            content = @Content(mediaType = "application/json"))
    @APIResponse(
            name = "Forbidden",
            description = "kann auch vorkommen, wenn mod_security zuschlägt",
    ...
    @APIResponse(
            ...
            responseCode = "500",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = MessagePayload.class)))
    public Response benutzerLoeschen(@Pattern(regexp = "^[abcdef\\d\\-]*$") @Size(max = 36) @PathParam(value = "uuid")
    @Pattern(regexp = "^[abcdef\\d\\-]*$", message = "uuid enthält ungültige Zeichen")
    @Size(max = 36, message = "uuid zu lang (max. 36 Zeichen)") final String uuid) {
