...
public interface StringLatinConstants {
    ...
    String DIACRITICS = A1 + A2 + B + C + D + E + F + G + H + I1 + I2 + J + K + L + M + N + O1 + O2 + P + R + S + T + TH
            + U + W + X + Y1 + Y2 + Z;

    // in der UI werden einige diakritische Zeichen nicht richtig dargestellt, da
    // sie aus 2 UI-Zeichen zusammengesetzt
    // sind.
    // da das vermutlich niemandem auffällt, werden für die Bildschirmtastatur hier
    // einige zulässige Zeichen entfernt
    // Das sind genau die Zeichen, die auch in pkg_oasis_utils.transliterate
    // Probleme machen.
    ...
    /**
     * Alle Buchstaben und diakritischen Zeichen aus StringLatin, alle Ziffern,
    ...
    /**
     * Alle Buchstaben und diakritischen Zeichen aus StringLatin, alle Ziffern,
     * Minus, Unterstrich, Punkt, Komma, Apostrophe und alle Arten von whitespace
     */
