...
public class SPARouteFilter {
	...
	private static final String APP_PLUS_API_PREFIX = "/bv-admin" + API_PREFIX;

	private static final String DEFAULT_APP = "/bv-admin/";
		...
		if (path.startsWith(APP_PLUS_API_PREFIX)) {
			...
			if (this.doesNotNeedRedirect(path)) {
				...
				if (path.startsWith(DEFAULT_APP)) {
					...
					// I0094: deep-Angular-Router-Links (z.B. /bv-admin/users/) müssen zur SPA Grund-URL
	...
	boolean doesNotNeedRedirect(final String path) {
		...
		if (FILE_NAME_PREDICATE.test(path)) {
			...
			LOGGER.info("(3-2) kein Umleiten von statischen files aus src/main/resources/META-INF/resources/bv_admin/");
			return true;
		}
