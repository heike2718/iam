...
public class MailversandProcessor {
    ...
    public void processMailversandauftrag() {
        ...
        if (nextMailversandgruppe == null) {
            ...
            if (!persistenterMailversandAuftrag.getStatus().isCompleted()) {
                ...
                // Es könnte sein, dass es gar keine Gruppe (mehr) gibt. Dann muss der Job auch
                // beendet werden, da sonst
                // ein Job
                // alle weiteren Aufträge blockiert

                // normalerweise wird bereits nach dem Versenden geschaut, ob anzahlEmpfaenger
                // == anzahlVersendet und
                // dann beendet,
                // aber zwischenzeitlich können sich USER in der DB verabschiedet haben und dann
                // ist anzahlVersendet
                // stets kleiner
                // als anzahlEmpfaenger. Für diesen Fall dauert es noch einen Jobzyklus, um den
                // Versandauftrag als
                // beendet zu
                // markieren
        ...
        if (persistenterMailversandAuftrag.getStatus() == Jobstatus.WAITING) {
            ...
            LOGGER
                    .info("starte Mailversandauftrag {}: anzahl Empfänger={}", persistenterMailversandAuftrag.getUuid(),
        ...
        try {
            ...
            persistenterMailversandAuftrag = updateMailversandauftragUndGruppe(persistenterMailversandAuftrag,
                    persistenteMailversandGruppe, nextMailversandgruppe.getEmpfaengerEmails().size(),
                    Jobstatus.COMPLETED);
            ...
            LOGGER
                    .error("Beim Mailversand an Gruppe {} ist ein Fehler aufgetreten: {}",
                            persistenteMailversandGruppe.getUuid(), e.getMessage(), e);
            persistenterMailversandAuftrag = updateMailversandauftragUndGruppe(persistenterMailversandAuftrag,
            ...
            LOGGER
                    .error("Unerwartete Exception beim Mailversand an Grupppe {}: {}",
                            persistenteMailversandGruppe.getUuid(), e.getMessage(), e);
        } finally {
    ...
    void markCompleted(final PersistenterMailversandauftrag versandauftrag) {
        ...
        LOGGER
                .info("Mailversandauftrag {} als beendet markiert: status={}, anzahlEmpfaenger={}, anzahlVersendet={}",
                        StringUtils.abbreviate(versandauftrag.getUuid(), 11), status,
                        versandauftrag.getAnzahlEmpfaenger(), versandauftrag.getAnzahlVersendet());
    ...
    void sendeMails(final PersistenterMailversandauftrag persistenterMailversandAuftrag,
            final Mailversandgruppe gruppe) {

        AuthAdminMailDto mailDto = new AuthAdminMailDto()
                .withAttachSpammailhinweis(true)
                .withBccEmpfaenger(gruppe.getEmpfaengerEmails())
                .withBetreff(persistenterMailversandAuftrag.betreff)
