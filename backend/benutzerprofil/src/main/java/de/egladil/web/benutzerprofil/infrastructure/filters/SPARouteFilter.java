...
/**
 * SPARouteFilter. Das Backend served die Angular-Anwendung. Daher müssen
 * statischen Webseiten woanders gesucht werden, als die Quarkus-app.
 */
public class SPARouteFilter {
        ...
        if (path.startsWith(APP_PLUS_API_PREFIX)) {
            ...
            if (this.doesNotNeedRedirect(path)) {
                ...
                if (path.startsWith(DEFAULT_APP)) {
                    ...
                    // I0094: deep-Angular-Router-Links (z.B. /benutzerprofil/user/) müssen zur SPA
                    // Grund-URL
                    // (/benutzerprofil/) umgeleitet werden. Danach übernimmt wieder das
                    // Angular-Routing
    ...
    boolean doesNotNeedRedirect(final String path) {
        ...
        if (FILE_NAME_PREDICATE.test(path)) {
            ...
            LOGGER
                    .info("(3-2) kein Umleiten von statischen files aus src/main/resources/META-INF/resources/benutzerprofil/");
            return true;
        }
