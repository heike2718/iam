...
import { ClientCredentials, ClientInformation, createHashForRedirectUrl } from '../../../model/auth.model';
...
export const loginSignUpFeature = createFeature({
    ...
    reducer: createReducer<SignUpState>(
        ...
        on(loginSignupActions.sIGN_UP, state => {
            return { ...state, submittingForm: true };
        }),
        on(loginSignupActions.lOG_IN, state => {
            return { ...state, submittingForm: true };
        }),
        on(loginSignupActions.lOG_IN_SIGN_UP_SUCCESS, (state, action) => {
            ...
            return {
                ...
                submittingForm: false,
                clientCredentials: undefined,
                redirectUrl: state.clientCredentials?.redirectUrl + hash,
            };
        }),
        on(loginSignupActions.rESET_LOGIN_SIGNUP_STATE, () => initialState)
    ),
});
