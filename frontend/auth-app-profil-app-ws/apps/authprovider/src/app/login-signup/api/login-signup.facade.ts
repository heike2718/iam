...
import { ClientCredentials, ClientInformation, LoginCredentials } from '../../model/auth.model';
...
export class LoginSignupFacade {
    ...
    clientCredentials$: Observable<ClientCredentials> = filterDefined(
        this.#store.select(fromLoginSignup.clientCredentials)
    );

    clientInformation$: Observable<ClientInformation | undefined> = this.#store.select(
        fromLoginSignup.clientInformation
    );
    ...
    resetState(): void {
        this.#store.dispatch(loginSignupActions.rESET_LOGIN_SIGNUP_STATE());
    }
}
