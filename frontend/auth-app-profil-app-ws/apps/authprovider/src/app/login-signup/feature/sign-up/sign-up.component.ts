...
import {
    NAME_REGELN,
    REG_EXP_INPUT_SECURED,
    REG_EXP_LOGIN_NAME,
    SIGNUP_SUCCESS_DIALOG_TEXT,
    trimFormValues,
} from '@ap-ws/common-utils';
import { LoginSignupFacade } from '../../api/login-signup.facade';
import { SignUpFormModel, SignUpCredentials } from '../../model/login-signup.model';
import { ClientCredentials, ClientInformation } from '../../../model/auth.model';
...
export class SignUpComponent implements OnInit, OnDestroy {
    ...
    infoBenutzerkontoDialog = inject(MatDialog);

    ngOnInit(): void {
        this.#createForm();

        const routeSubscription = this.#route.queryParams.subscribe(params => this.#loadClientInfo(params));
    ...
    #loadClientInfo(params: Params) {
        // https://mathe-jung-alt.de/authprovider/signup?accessToken=410b0d049290454a99f1cdfe82876862&state=signup&nonce=LEHRER-OXEO0PY0&redirectUrl=https:%2F%2Fmathe-jung-alt.de%2Fmkv-app
        // https://mathe-jung-alt.de/authprovider/signup?accessToken=9566d324574b4750a7a7ff06e6916712&state=signup&redirectUrl=https:%2F%2Fmathe-jung-alt.de%2Fmja-app%2F
    ...
    #updateForm(clientInformation: ClientInformation): void {
        if (clientInformation) {
            if (clientInformation.loginnameSupported) {
                ...
                const loginNameControl = this.#fb.control('', [
                    Validators.pattern(REG_EXP_LOGIN_NAME),
                    Validators.maxLength(255),
                ]);

                this.benutzerForm.addControl('loginName', loginNameControl);
            }

            if (clientInformation.namenRequired) {
                this.showVornameNachnameControl = true;

                const vornameControl = this.#fb.control('', [
                    Validators.required,
                    Validators.maxLength(100),
                    Validators.pattern(REG_EXP_INPUT_SECURED),
                ]);
                const nachnameControl = this.#fb.control('', [
                    Validators.required,
                    Validators.maxLength(100),
                    Validators.pattern(REG_EXP_INPUT_SECURED),
                ]);

                this.benutzerForm.addControl('vorname', vornameControl);

                this.benutzerForm.addControl('nachname', nachnameControl);
            }
        }
    }
