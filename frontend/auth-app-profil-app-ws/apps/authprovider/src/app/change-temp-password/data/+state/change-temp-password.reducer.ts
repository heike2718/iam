...
export const changeTempPasswordFeature = createFeature({
    ...
    reducer: createReducer<ChangeTempPasswordState>(
        initialState,
        on(changeTempPasswordActions.cHANGE_TEMP_PASSWORD, state => {
            return {
                ...state,
                submittingForm: true,
                tempPasswordSuccess: false,
        ...
        on(changeTempPasswordActions.tEMP_PASSWORD_CHANGED, (state, action) => {
            return {
                ...
                tempPasswordSuccessMessage: action.payload.message.message,
            };
        }),
        on(changeTempPasswordActions.rESET_CHANGE_TEMP_PASSWORD_STATE, () => initialState)
    ),
});
