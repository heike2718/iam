...
import { ChangeTempPasswordFacade } from '../../api/change-temp-password.facade';
...
export class ChangeTempPasswordComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        if (this.tokenId === null) {
            this.#showDialog(
                'Fehler!',
                'Der aufgerufene Link ist ungültig. Bitte kopieren Sie den Link vollständig oder klicken Sie ihn nochmals an.'
            );
        }
        ...
        const tempPasswordSuccessMessageSubscription =
            this.changeTempPasswordFacade.tempPasswordSuccessMessage$.subscribe(message => {
                if (message) {
    ...
    #createForm() {
        this.parentForm = this.#formBuilder.group({
            ...
            tempPassword: [
                '',
                [Validators.required, Validators.maxLength(36), Validators.pattern(REG_EXP_EINMALPASSWORT)],
            ],
            kleber: [''],
        });
