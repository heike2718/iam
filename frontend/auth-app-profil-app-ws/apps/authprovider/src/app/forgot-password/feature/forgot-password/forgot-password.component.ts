...
import { ForgotPasswordFacade } from '../../api/forgot-password.facade';
...
export class ForgotPasswordComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        const submittingSubscription = this.forgotPasswordFacade.submittingForm$.subscribe(
            submitting => (this.#submittingForm = submitting)
        );
        const tempPasswordSuccessMessageSubscription = this.forgotPasswordFacade.tempPasswordSuccessMessage$.subscribe(
            message => {
                if (message) {
    ...
    submit(): void {
        if (this.orderPwdForm.valid) {
            ...
            this.forgotPasswordFacade.orderTempPassword({ ...tempPasswordCredentials, formStartTs: this.#formStartTs });
        }
    }
